...
- [x] **9. O_EXCL, O_NONBLOCK** в vfs_open — parent-lock для O_EXCL race `bsd/vfs/vfs.c:454` (spinlock вокруг lookup+create), O_NONBLOCK уже в `vfs_read`/`vfs_write` через `poll`+`EAGAIN`
      СДЕЛАНО 2026-09-19: `vfs_open_lock` (irqsave) защищает lookup→O_EXCL-check→create как атомарную секцию.
- [ ] **10. mmap/munmap/mprotect** (vnode_ops.mmap уже задекларирован)
- [ ] **11. fcntl** (F_DUPFD/F_GETFD/F_SETFD/F_GETFL/F_SETFL)
- [x] **12. waitpid WCONTINUED + killpg + setsid/setpgid** — `WCONTINUED` (8) + `WIFCONTINUED==0xFFFF`, `killpg`, `setsid`, `setpgid`, `continued` flag + wake parent для stop/continue
      СДЕЛАНО 2026-09-19: `bsd/proc/proc.c:844` + `bsd/signal/signal.c` + `bsd/sys/sys_signal.c` + `bsd/sys/sys_process.c` + `bsd/include/bsd/proc.h:278`.

## Статус

Сделано: задачи 1-9,12 + `arm64 PMM bitmap`, `heap HAL` (`mk/vm/heap.c` + `mk/include/heap.h`), `stack protector` (`-fstack-protector-strong` + `mk/lib/stack_protector.c`) + `HOST_CFLAGS+=-fsanitize=address,undefined`, `ISIG` в TTY (`bsd/tty/tty.c:290`), `.clang-format`, `-Werror -MMD` в `Makefile:118`.
Следующая: задача 10 (mmap/munmap/mprotect).
